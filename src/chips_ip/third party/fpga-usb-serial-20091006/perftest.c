...
static void usb_lossrx_cb(struct libusb_transfer *urb)
{
	...
	*(int *)(urb->user_data) += urb->actual_length;
...
static void test_libusb(int vendorid, int productid, int busnum, int devnum)
{
	...
	while (bcnt[0] < BLASTRXSIZE)
		libusb_handle_events(ctx);
	elapsed = timer_elapsed(&tv);
	for (i = 0; i < USB_NUMURB; i++) {
		r = libusb_cancel_transfer(urb[i]);
		if (r)
			exit(1);
	}
	while (1) {
		for (i = 0; i < USB_NUMURB; i++)
			if (urb[i]->status != LIBUSB_TRANSFER_CANCELLED)
				break;
		libusb_handle_events(ctx);
		if (i == USB_NUMURB)
			break;
	...
	while (1) {
		for (i = 0; i < USB_NUMURB; i++)
			if (urb[i]->status != LIBUSB_TRANSFER_TIMED_OUT)
				break;
		libusb_handle_events(ctx);
		if (i == USB_NUMURB)
			break;
