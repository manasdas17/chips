...
class Plugin:
    ...
    def write_stimulus(self, stream): 
        ...
        self.source.extend([
            ...
            "data_type buffer_{0} = ".format(stream.get_identifier()),
            "  {0, true};",
    ...
    def write_unary(self, stream): 
        ...
        self.source.extend([
            ...
            "  }",
            "  data.value = {0};".format(
                functions[stream.function].format(
                    stream.a.get_identifier(),
                    stream.constant, 
                ),
            ),
            "  data.stalled = data_1_{0}.stalled;".format(stream.get_identifier()),
            "  if(debug){",
            "   cerr << {0} << \" binary \" << data.value << \" \" << data.stalled << endl;".format(stream.get_identifier()),
            "  }",
            "  return data;",
