"""VHDL generation of the Binary Modulo operator"""
...
"  process",
"    variable MODULO : unsigned({0} downto 0);".format(bits - 1),
"  begin",
...
"            SIGN_{0} <= STREAM_{1}({3});".format(identifier, identifier_a, identifier_b, bits_a-1, bits_b-1),
...
"",
"      when WRITE_Z =>",
"        MODULO := unsigned(SHIFTER_{0})/2;".format(identifier),
"        if SIGN_{0} = '1' then --if negative".format(identifier),
"          STREAM_{0} <= std_logic_vector( 0 - MODULO);".format(identifier),
"        else",
"          STREAM_{0} <= std_logic_vector( MODULO);".format(identifier, bits),
"        end if;",
...
"",
"  --subtractor",
"  REMAINDER_{0} <= std_logic_vector(unsigned(SHIFTER_{0}) - unsigned(B_{0}));".format(identifier, bits),
"",
    ]
