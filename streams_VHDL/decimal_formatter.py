...
def write(stream):
    ...
    "  signal COUNT_{0}      : integer range 0 to {1};".format(identifier, bits_a-1),
...
"          if STREAM_{0}({1}) = '1' then".format(identifier_a, bits_a-1),
'            BINARY_{0} <= std_logic_vector(-signed(STREAM_{1}));'.format(identifier, identifier_a),
"          else",
"            BINARY_{0} <= STREAM_{1};".format(identifier, identifier_a),
"          end if;",
"          COUNT_{0} <= {1};".format(identifier, bits_a-1),
...
"        CARRY_{0}(0) := BINARY_{0}({1});".format(identifier, bits_a-1),
