...
def translate_coordinates(ra_stream, dec_stream):
    ...
    Process(20, #gives integer range -512 to 511
        ...
        #    Break(),
            ...
            #increment siderial time
            lst.set(lst+1),
            If(lst==512, lst.set(0)),
...
#simulation_model=System(DecimalPrinter(svga_stream))
#simulation_model.test("fixed test ", stop_cycles=10000000)
...
#Join the elements together into a system
svga_stream = translate_coordinates(ra_stream, dec_stream)
synthesis_model=System(SVGA(Resizer(svga_stream, 8)))
...
simulation_plugin.xilinx_build("xc3s200-4-ft256")
